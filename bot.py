...
def prepare_promo(inp, out):
    cmd = [FFMPEG, '-i', inp, '-vf', 'scale=640:360', '-c:v', 'libx264',
           '-preset', 'ultrafast', '-c:a', 'aac', '-ar', '44100', '-ac', '2', '-y', out]
    subprocess.run(cmd, capture_output=True)
    return out

def trim_video(inp, start, end, out):
    cmd = [FFMPEG, '-i', inp, '-ss', str(to_sec(start)), '-to', str(to_sec(end)), '-c', 'copy', '-y', out]
    subprocess.run(cmd, capture_output=True)
    return out
...
def insert_promo_at_time(main, promo, insert_sec, out):
    uid = out.replace('.mp4', '')
    part1, part2, merged1 = f"{uid}_p1.mp4", f"{uid}_p2.mp4", f"{uid}_m1.mp4"
    dur = get_duration(main)
    if insert_sec >= dur: insert_sec = dur / 2
...
def add_promo_to_part(part, promo, promo_pos, promo_time, out):
    if promo_pos == "start": merge_fast(promo, part, out)
    elif promo_pos == "end": merge_fast(part, promo, out)
    elif promo_pos == "custom" and promo_time: insert_promo_at_time(part, promo, to_sec(promo_time), out)
    else: insert_promo_at_time(part, promo, get_duration(part)/2, out)
    return out

def split_video(inp, max_mb=MAX_FILE_MB):
    size_mb = os.path.getsize(inp) / (1024*1024)
    if size_mb <= max_mb: return [inp]
    ...
    for i in range(n):
        ...
        subprocess.run([FFMPEG, '-i', inp, '-ss', str(i*part_dur), '-t', str(part_dur), '-c', 'copy', '-y', p], capture_output=True)
...
async def trim_choice(update, context):
    ...
    await q.answer()
    if q.data == "trim_yes":
        await q.edit_message_text("✂️ সময় দাও:\n`শুরু - শেষ`\nউদাহরণ: `1:30 - 5:45`", parse_mode="Markdown")
        return WAITING_TRIM
...
async def receive_promo_file(update, context):
    ...
    kb = [[InlineKeyboardButton("⏮️ শুরুতে", callback_data="pos_start")],
    ...
    try:
        ...
        for i, part in enumerate(parts, 1):
            ...
            if promo_path and os.path.exists(promo_path):
                ...
                if os.path.exists(promo_out): send_path = promo_out
            await message.reply_text(f"📤 পাঠাচ্ছি Part {i}/{total}...")
            with open(send_path, "rb") as f:
                await context.bot.send_video(chat_id=chat_id, video=f,
                    caption=f"🎬 Part {i}/{total}\n\n_BY : RH RATUL_", parse_mode="Markdown")
            if send_path != part: cleanup(send_path)
        await message.reply_text(f"✅ Done! {total}টা Part!\n\n_BY : RH RATUL_", parse_mode="Markdown")
...
conv = ConversationHandler(
    ...
    states={
        ...
        WAITING_TRIM: [CallbackQueryHandler(trim_choice, pattern="^trim_"), MessageHandler(filters.TEXT & ~filters.COMMAND, receive_trim)],
